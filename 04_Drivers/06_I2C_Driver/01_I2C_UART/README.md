<div align="center">

<div style="background: linear-gradient(135deg, #FF4500, #3A1500); padding: 30px 40px; border-radius: 16px; margin-bottom: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.3);">

<h1 style="color: white; margin: 0; font-size: 2.2em; letter-spacing: 1px;">🔌 I2C UART</h1>

<p style="color: rgba(255,255,255,0.85); margin: 10px 0 0 0; font-size: 1.1em;">I2C-to-UART bridge driver</p>

</div>

</div>

📍 `Workspace / Linux / 02_LKI_Explore / 00_Linux_Kernel / 04_Drivers / 01_CharDriver / 00_CharDriver / 02_I2C_Driver / 01_I2C_UART`

![Category](https://img.shields.io/badge/Category-Char%20Driver-FF4500?style=flat-square) ![Docs](https://img.shields.io/badge/Docs-6-2E8B57?style=flat-square)

---

<h2 style="color: #FF4500;">📄 Files</h2>

| | File | Type |
|:---:|:---|:---|
| 📄 | [01_Implement_I2C_Driver.md](01_Implement_I2C_Driver.md) | Markdown |
| 📄 | [01_MyUARTDriver.md](01_MyUARTDriver.md) | Markdown |
| 📄 | [02_Implement_I2C_Driver.md](02_Implement_I2C_Driver.md) | Markdown |
| 📄 | [02_TempSensor.md](02_TempSensor.md) | Markdown |
| 📄 | [03_DebugI2CDriver.md](03_DebugI2CDriver.md) | Markdown |
| 📄 | [04_ProbeNotWorkingI2C.md](04_ProbeNotWorkingI2C.md) | Markdown |

---

<div align="center">

<p style="color: #888; font-size: 0.9em;">[⬆️ Parent Directory](../README.md) &nbsp;|&nbsp; [🏠 Workspace Root](../../../../../../../../README.md)</p>

</div>

---

<div align="center">
<sub style="color: #666;">Auto-generated README — <b style="color: #FF4500;">Char Driver</b></sub>
</div>
